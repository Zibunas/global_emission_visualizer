...
def ac_line_add_to_fig(pm):
    ...
    if  pm.add_to_fig == True and pm.close_fig==True:
        pm.close_fig = False
        messagebox.showinfo('Conflict','You chose to add the next country to your current plot. Therefore, the last line plot will not be closed.')
...
def change_cut_off_criteria(pm):
    ...
    pm.cut_off_limit = get_cutoff_limits(pm, "Choose X, e.g, 2.5 for 2.5%. Only slices bigger than X% will be explicitly shown, all others will be aggregated in rest. (In addition, in the next window you are going to define the maximum amount of aggregated values.)", parent=canvas)
    pm.cut_off_sum_limit = get_cutoff_limits(pm, "Choose Y, e.g., 33 for 33%. The aggregated rest will be Y% at maximum. This will overrule the previous criterium in case there are too many small values", parent=canvas)
